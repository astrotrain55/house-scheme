...
export default class SchemeHouse {
  ...
  autoBoxes() {
    ...
    if (type === 'high') {
      if (boxesPosition === 'default') this.autoBoxesDefaultHigh(routingPosition);
      ...
      const options = {
        search: {
          ...
          leftPorch: porchNumber - 1,
          rightPorch: porchNumber + 1,
          leftPart,
          rightPart,
  ...
  newLogicAutoBoxes(types, params) {
    ...
    const floors = porch.floors.filter((f) => f.visible && f.type === 'default');
    if (params.isBottom) floors.reverse();
    const apartmentsCount = floors.reduce((acc, floor) => acc + floor.appt, 0);
    const maxInternet = params.maxInternet < apartmentsCount ? 150 : params.maxInternet;
    ...
    if (params.direction === 'center-left' && params.parts.left.length && params.maxCountPorchLeft) {
    ...
    } else if (params.direction === 'center-left' && params.parts.right.length && params.maxCountPorchRight) {
      ...
      params.maxCountPorchRight = params.maxCountPorchRight
        ...
        : this.getMaxCountPorchDefault(params.type);
    } else if (params.direction === 'center-right' && params.parts.right.length && params.maxCountPorchRight) {
      params.porchNumber = params.parts.right.shift();
      params.maxCountPorchRight = params.maxCountPorchRight
        ? params.maxCountPorchRight - 1
        : this.getMaxCountPorchDefault(params.type);
    } else if (params.direction === 'center-right' && params.parts.left.length && params.maxCountPorchLeft) {
    ...
    if (!isFantom && ['center-left', 'center-right'].includes(params.direction)) {
  ...
  newLogicParsePorch(types, options) {
    const boxes = [];
    let remainderMaxInternet = options.maxInternet;
    let remainderInternet = options.visibleInternet ? options.apartmentsCount : 0;
    let remainderKtv = options.visibleKtv ? options.apartmentsCount : 0;

    recursiveHandler();

    function recursiveHandler(start = 0) {
      const typesBoxes = remainderMaxInternet === 150 ? config.typesBoxes : types;
      const isParsePorch = () => {
      ...
      for (let i = start; i < typesBoxes.length; i++) {
        ...
        if (remainderInternet > 0 || remainderKtv > 0) {
          if ((nextType && remainderMaxInternet >= nextType.internet) || remainderKtv > 0) {
  ...
  autoBoxesTech(options) {
    ...
    function getTypes(box) {
      const types = config.typesBoxes.map((type) => {
        type.color = randomColor();
        return type;
      });
      if (box) types[0] = box;
    ...
    const { boxes, internet, ktv } = this.parsePorch(porch, apptCount, options.typesBoxes, options);
    ...
    floors.forEach((floor, index) => {
      ...
      if (!startTech) {
        if (prevTech.type === 'tech' && floor.type === 'default') startTech = prevTech;
      }
      if (startTech && !endTech) {
        if (prevTech.type === 'default' && floor.type === 'tech') endTech = floor;
      }
    });
    ...
    if (countBoxes === 1) {
      if (options.search.type === 'center-left' && options.search.rightPart.length) {
      ...
      } else if (options.search.type === 'center-right' && options.search.leftPart.length) {
        ...
        options.typesBoxes = options.search.type !== 'right' ? getTypes() : typesBoxes;
  ...
  autoBoxesTechHigh(routingPosition) {
    const isVisible = (ess) => ess.visible;
  ...
  generateBoxesTech(boxes, originBox, startTech, endTech) {
    boxes.forEach((typeBox, index) => {
  ...
  autoBoxesDefaultHigh(routingPosition) {
    const isVisible = (ess) => ess.visible;
    ...
    this.porches.filter(isVisible).forEach((porch) => {
      ...
      const floors = porch.floors.filter(isVisible).filter((f) => f.type === 'default');
      if (routingPosition === 'bottom') floors.reverse();
      let apptCount = 0;
  ...
  generateBoxesDefault(typesList, floors) {
  ...
  movingTechFloor(dir, floor) {
    const [firstPorch] = this.porches;
    const indexFloor = firstPorch.floors.findIndex((f) => f.name === floor.name);
  ...
  calculate() {
    ...
    this.house = new House({
      id: house.UF_XML_ID,
      nok: Boolean(house.UF_APT_NUMBERS),
      type,
    });
  ...
  calculatePorches(countFloors, numbers, house) {
    return this.options.porches.map((mapPorch, i) => {
      const numbersPorch = numbers[mapPorch.UF_NUMBER];
      ...
      const floors = this.calculateFloors(porch, mapPorch, countFloors, numbersPorch.floors);
  ...
  calculateFloors(porch, mapPorch, countFloors, numbersFloor) {
    const floors = [];
  ...
  calculateBoxes(mapPorch, floorNumber) {
    const boxes = [];
  ...
  getListTechFloors(porch, mapPorch) {
    return {
      tower: new Floor({
        name: 'Надстройка',
